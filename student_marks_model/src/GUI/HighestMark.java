...
public class HighestMark extends JFrame {
	...
	public HighestMark() {
		...
		JButton highMarksbtn = new JButton("Submit");
		highMarksbtn.setBounds(113, 91, 89, 23);
		contentPane.add(highMarksbtn);
		...
		highMarksbtn.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
            	result.setText("");
            	if(year.getText() !=null && subcode.getText() != null) {
            		Marks marks = new FetchFromDatabase().getHighMarks(subcode.getText().trim().toLowerCase(), year.getText().trim().toLowerCase());
            		result.setText("Roll: "+marks.getRoll()+"\nYear: "+marks.getSemYear()+"\nPaper Code: "+marks.getPaperCode()+
                			"\nPaper Title: "+marks.getPaperTitle()+"\nHighest: "+marks.getObMark());
            	}
            }
        });
